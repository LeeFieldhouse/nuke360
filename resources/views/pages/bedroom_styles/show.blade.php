	...
	<section class="page-section bg-dark-alfa-{{$area->banner_opacity}} parallax-3" data-background="{{$style->featured_image}}">
		<div class="relative container align-left">
			...
			<div class="row">
				...
				<div class="col-md-8">
					<h1 class="hs-line-11 font-alt mb-20 mb-xs-0">{{$style->name}} Bedrooms</h1>
				...
				<div class="col-md-4 mt-30">
					<div class="mod-breadcrumbs font-alt align-right">
						...
						<a href="{{route('bedroom.styles.index')}}">/ BEDROOM STYLES</a>
						<a href="{{\Illuminate\Support\Facades\URL::current()}}"> / {{$style->name}} Bedrooms</a>
					</div>
				</div>
			</div>
		</div>
	...
	<section class="mt-40">
		<div class="container relative text-center">
			<div class="font-alt mb-20">
				<a href="{{route('bedroom.styles.index')}}"> << Back to styles</a>
			</div>
			<!-- Works Filter -->
			<div class="works-filter font-alt align-center">
				<a href="{{$style->getUrl()}}" class="{{blank(request('range')) ? 'active' : ''}}">All ranges in {{$style->name}}</a>
				@foreach($ranges as $range)
				<a href="{{route('bedroom.styles.show', [$style->id, $style->slug, 'range' => $range->id])}}" class="{{request('range') == $range->id ? 'active' : ''}}">{{$range->name}}</a>
				@endforeach
			</div>
			<!-- End Works Filter -->
...
{{--									<a href="{{route('bedroom.ranges.show', [$image->range->id, $image->range->slug])}}" class="work-descr" style="padding-bottom: 25px!important;">--}}
